...
void main(){
  /*
   ...
   // create topographical shape
    ...
    // calculate the width
    float widthSize = fwidth(shapeFormula);
    // fix jagged edges by using smooth step
    float shape = smoothstep(0.5 - widthSize, 0.5 + widthSize, shapeFormula);
    // white color
    vec3 whiteColor = vec3(1.0);
    // uv color
    vec3 uvColor = vec3(vUv + vec2(cos(uTime), sin(uTime)) * 0.5, 0.5);
    // clamp the color
    uvColor = clamp(uvColor, 0.0, 1.0);
    // combine the colors
